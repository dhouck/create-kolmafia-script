...
Create a kolmafia script with ease, with some additional patches
...
### This version
#### One-time setup
To use *this* package with the DanielH changes, you need to use the `@dhouck/create-kolmafia-script` package from GitHub Packages.  The instructions for how to do this vary between package managers; the ones below work for yarn.

First, check your global Yarn version:
```bash
yarn --version
```
If the result starts with version 1 (probably 1.22.22), then run
```bash
yarn config set "@dhouck:registry" https://npm.pkg.github.com
```
If the result starts with something between 2 and 4 (most likely 4.18.0), then run
```bash
yarn config set -H npmScopes.dhouck.npmRegistryServer https://npm.pkg.github.com
```
If the version starts with 5 or 6, then you either live at least a month in the future from when I wrote this, or you are using a preview version of ZPM and hopefully know how it works better than I do.
